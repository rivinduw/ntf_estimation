                ...
                #     print(net_output[0].size())
                    ...
                    for i in range(4):
                        ...
                        try:
                            ...
                            print(e)
                        try:
                            ax = pd.DataFrame(10000*target[0,:,i*self.variables_per_segment+1]).plot()
                            pd.DataFrame(10000*preds[0,:,i*self.variables_per_segment+1]).plot(ax=ax)
                            #pd.DataFrame(10000*src[0,:,i*self.variables_per_segment]).plot(ax=ax)
                            ax.legend(['target','pred','input'])
                            ...
                            print(e)
                        try:
                            ax = pd.DataFrame(10000*target[0,:,i*self.variables_per_segment+2]).plot()
                            pd.DataFrame(10000*preds[0,:,i*self.variables_per_segment+2]).plot(ax=ax)
                            #pd.DataFrame(10000*src[0,:,i*self.variables_per_segment]).plot(ax=ax)
                            ax.legend(['target','pred','input'])
                    ...
                    try:
                        ...
                        ax = pd.DataFrame(5000*target[0,:,self.variables_per_segment+4]).fillna(0.0).plot()
                        pd.DataFrame(5000*preds[0,:,self.variables_per_segment+4]).fillna(0.0).plot(ax=ax)
                        #pd.DataFrame(5000*src[0,:,self.variables_per_segment+4]).fillna(0.0).plot(ax=ax)
                        ax.legend(['target','pred'])
                        wandb.log({"offramp": ax})
                        ax2 = pd.DataFrame(5000*target[0,:,self.variables_per_segment*3+3]).fillna(0.0).plot()
                        pd.DataFrame(5000*preds[0,:,self.variables_per_segment*3+3]).fillna(0.0).plot(ax=ax2)
